...
const Settings: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Load saved theme color from localStorage if available
  ...
  // Updated function to handle color change and notify other components
  const handleColorChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    window.dispatchEvent(
      new CustomEvent('themeColorChanged', { 
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      const res = await axios.patch(`http://127.0.0.1:8000/users/${currentUser.id}/`, editForm);
  ...
  const handleRoleChange = async (userId: number, newRole: string) => {
    try {
      await axios.patch(`http://127.0.0.1:8000/users/assign-role/${userId}/`, { role: newRole });
      ...
      // If changing current user's role, update that too
