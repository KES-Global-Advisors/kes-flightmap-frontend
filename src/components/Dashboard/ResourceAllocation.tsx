      ...
      <table className="min-w-full">
        <thead>
          <tr className="bg-gray-50">
            ...
            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Current Tasks</th>
            ...
            <tr key={idx}>
              ...
              <td className="px-6 py-4 whitespace-nowrap">
                <span className={`px-2 py-1 rounded-full text-xs ${
                  workload.current_tasks > 5 ? 'bg-red-100 text-red-800' :
                  workload.current_tasks > 3 ? 'bg-yellow-100 text-yellow-800' :
                  'bg-green-100 text-green-800'
                }`}>
                  {workload.current_tasks > 5 ? 'Overloaded' :
                   workload.current_tasks > 3 ? 'Heavy' : 'Optimal'}
                </span>
              </td>
            </tr>
          ))}
